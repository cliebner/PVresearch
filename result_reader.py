...
def get_diffuse_tracking(tracking_df, flat_df):
    # Compare "effective" irradiance from two PVsyst simulations, where "effective" accounts for linear effect
    # of 3D near shadings construction & for IAM (impt at low sun angles!):
    # Outputs: a dataframe with diffuse metrics concatenated onto input tracking_df
    # Inputs: tracking_df = dataframe of PVsyst results for system with 1-axis tracking
    #         flat_df     = dataframe of PVsyst results for system with fixed 0 deg tilt

    diffuse_tracking_gain = flat_df[PVSYST_CONVENTION[E_GLOBAL]] - tracking_df[PVSYST_CONVENTION[E_GLOBAL]]
    is_diffuse_tracking_better = (diffuse_tracking_gain > 1.0) & (tracking_df[PVSYST_CONVENTION[E_GLOBAL]] > 1.0)
    diffuse_tracking_gain_ratio = diffuse_tracking_gain / tracking_df[PVSYST_CONVENTION[E_GLOBAL]]

    # Creates new series that represents the simple diffuse tracking approach
    # (i.e. control tracker to zero degrees during diffuse hour):
    when_flat = pd.Series([0]*8760, index=flat_df.index)[is_diffuse_tracking_better]
    when_tracking = tracking_df.loc[(is_diffuse_tracking_better == False), PVSYST_CONVENTION[PANEL_PHI]]
